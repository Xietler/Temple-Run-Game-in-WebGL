ZERO.Camera = ZERO.Class(
{
	...
	{
		this.eye = new ZERO.Vec3(0, 0, 0);
		this.lookDir = new ZERO.Vec3(0, 1000, 0);
		this.updateView();
		if(!(w && h))
		{
			w = 800;
	...
	updateView : function()
	{
		...
		this.modelViewMatrix = ZERO.makeLookAt(eye[0], eye[1], eye[2], centerX, centerY, lookDir[2], dirBackX, dirBackY, len);
	},
	...
	turnRight : function(rad)
	{
		...
		var v = ZERO.mat2MulVec2(ZERO.mat2Rotation(rad), this.lookDir).data;
		d[0] = v[0];
		d[1] = v[1];
	},
	...
	{
		var d = this.lookDir.data;
		var v = ZERO.mat2MulVec2(ZERO.mat2Rotation(rad), new ZERO.Vec3(0, 1, 0)).data;
		d[0] = v[0];
		d[1] = v[1];
	},
	...
	resize : function(w, h)
	{
		this.projectionMatrix = ZERO.makePerspective(this.fovyRad, w / h, this.zNear, this.zFar);
	}
